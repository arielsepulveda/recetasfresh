// Service that crawls HelloFresh recipe card metadata and PDF files.
...
const downloadRecipeCards = async function (items) {
  ...
  if (Array.isArray(items) && items.length > 0) {
    ...
    for (const item of downloadedCards) {
      try {
        const sanitizedName = sanitizeFilename(item.name);
        const filePath = `${recipeCardSaveDirectory}/${sanitizedName}.pdf`;
        ...
        console.log(
          `- Error saving "${item.name}": ${err.message}. Continuing with next file...`
        );
        failedCount++;
      }
    }
    ...
    console.log(
      `- Save completed: ${savedCount} files saved, ${failedCount} failed.`
    );
  }
};
